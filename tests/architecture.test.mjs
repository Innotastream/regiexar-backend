...
test("le backend 0.14.3 conserve la file Codex et le schéma 13", async () => {
  const [index, domains, manifest] = await Promise.all([read("api/v1/index.php"), read("api/v1/domains.php"), read("manifest.json")]);
  assert.match(index, /XAR_BACKEND_VERSION = '0\.14\.3'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-3-1-3-map-vision-sync-release-20260904-1'/);
  ...
  assert.match(index, /state_schema_version = 13/);
  assert.match(domains, /XAR_SESSION_SCHEMA_VERSION = 13/);
  assert.match(domains, /legacyStateToDomains/);
  assert.equal(JSON.parse(manifest).backendVersion, "0.14.3");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "3.1.3");
  assert.equal(JSON.parse(manifest).databaseSchemaVersion, 15);
...
test("la commande ciblée déplace les tokens MJ et Joueur sans élargir les droits Joueur", async () => {
  ...
  assert.match(command, /'revision' => \(int\) \(\$records\[\$tokenKey\]\['revision'\] \?\? 0\) \+ 1/);
...
test("seule la version MSIX annoncée peut utiliser l’API", async () => {
  ...
  assert.match(index, /XAR_RELEASE_ANNOUNCEMENT_VERSION = '3\.1\.3'/);
  assert.equal(JSON.parse(manifestSource).announcedApplicationVersion, "3.1.3");
  ...
  assert.equal("3.1.2" === "3.1.3", false, "le MSIX précédent doit être refusé");
  assert.equal("3.1.3" === "3.1.3", true, "seul le MSIX annoncé doit franchir le verrou");
  assert.equal("3.1.4" === "3.1.3", false, "un MSIX futur non annoncé doit être refusé");
