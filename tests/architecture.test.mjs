...
test("le backend partage la file Codex, porte les nouveaux schémas et conserve le domaine chance", async () => {
  ...
  assert.match(index, /XAR_BACKEND_VERSION = '0\.18\.11'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-3-4-9-wall-shadow-vision-edge-20260926-1'/);
  ...
  assert.equal(JSON.parse(manifest).backendVersion, "0.18.11");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "3.4.9");
...
test("la santé reste publique mais seule la version courante peut se connecter", async () => {
  ...
  assert.equal(manifest.announcedApplicationVersion, "3.4.9");
  assert.deepEqual(manifest.allowedApplicationVersions, ["3.4.9"]);
