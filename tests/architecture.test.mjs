...
test("le backend partage la file Codex, porte les nouveaux schémas et conserve le domaine chance", async () => {
  ...
  assert.match(index, /XAR_BACKEND_VERSION = '0\.17\.2'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-3-3-7-sync-anomaly-diagnostics-candidate-20260922-1'/);
  ...
  assert.equal(JSON.parse(manifest).backendVersion, "0.17.2");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "3.3.7");
...
test("la santé reste publique mais seule la version courante peut se connecter", async () => {
  ...
  assert.equal(manifest.announcedApplicationVersion, "3.3.7");
  assert.deepEqual(manifest.allowedApplicationVersions, ["3.3.7"]);
