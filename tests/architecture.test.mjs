...
test("le backend 0.14.13 conserve la file Codex et porte le schéma 16", async () => {
  const [index, domains, manifest] = await Promise.all([read("api/v1/index.php"), read("api/v1/domains.php"), read("manifest.json")]);
  assert.match(index, /XAR_BACKEND_VERSION = '0\.14\.13'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-3-1-13-token-groups-layers-and-clones-release-20260907-1'/);
  ...
  assert.equal(JSON.parse(manifest).backendVersion, "0.14.13");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "3.1.13");
...
test("les attaques ciblées et opposées restent autoritaires sans divulguer l’armure", async () => {
  ...
  assert.match(attack, /\$command === 'token\.attack\.resolve'/);
  assert.match(attack, /\$arguments\['confirmed'\][\s\S]*?attack_confirmation_required/);
  assert.match(attack, /if \(\$decision === 'approve'\)[\s\S]*?applyOnlineAttackDamage/);
  assert.match(attack, /else \{[\s\S]*?\$attack\['status'\] = 'rejected'/);
  assert.match(applyDamage, /onlineTokenIsDead\(\$token, \$controllerId !== ''\)[\s\S]*?target_already_defeated/);
  assert.match(applyDamage, /if \(\$requestedDamage <= 0\)[\s\S]*?attack_damage_not_effective/);
  assert.match(applyDamage, /'characterId' => is_array\(\$character\) \? \$characterId : ''/);
  assert.match(attack, /Cette cible est déjà morte\.[\s\S]*?target_already_defeated/);
  assert.match(attack, /!in_array\(\$decision, \['approve', 'reject'\], true\)[\s\S]*?invalid_attack_decision/);
  assert.doesNotMatch(attack, /=== 'reject' \? 'reject' : 'approve'/);
  assert.match(attack, /count\(\$pendingAttacks\) >= XAR_PENDING_ATTACK_MAXIMUM[\s\S]*?pending_attack_capacity_reached/);
  assert.match(attack, /count\(\$receipts\) >= XAR_ATTACK_RECEIPT_MAXIMUM[\s\S]*?attack_receipt_capacity_reached/);
  ...
  assert.match(online, /function onlinePendingAttackParties[\s\S]*?attack_parties_changed/);
  assert.match(online, /\(\$attack\['attackerRole'\] \?\? 'player'\) !== 'gm'[\s\S]*?opposition_target_no_longer_adverse/);
  ...
  assert.match(domains, /\['physical', 'magical', 'ignore'\]/);
});
...
test("la commande ciblée déplace les tokens MJ et Joueur sans élargir les droits Joueur", async () => {
  ...
  assert.match(command, /if \(!\$isGm && \(\$table\['tacticalSync'\]\['paused'\]/);
  assert.match(command, /\$requestedSceneId = trim/);
  assert.match(command, /\$requestedSceneId === '' \|\| \(!\$isGm && \$requestedSceneId !== \$sceneId\)/);
  assert.match(command, /\$moveSceneId = \$requestedSceneId/);
  assert.ok(command.indexOf("if ($moveSceneId === '')") < command.indexOf("onlineTokenDomainKey"));
  assert.match(command, /onlineTokenDomainKey\(\$moveSceneId/);
  assert.match(command, /\$initiativeKey = 'initiative:' \. \$moveSceneId/);
  ...
  assert.match(online, /\['ensure-player', 'admin\.character\.delete', 'token\.move', 'tokens\.transform', 'token\.clone', 'token\.conditions\.update', 'character\.conditions\.update', 'token\.resource\.adjust', 'action\.undo', 'token\.attack', 'token\.attack\.oppose', 'token\.attack\.resolve', 'ping'\]/);
...
test("seule la version MSIX annoncée peut utiliser l’API", async () => {
  ...
  assert.match(index, /XAR_RELEASE_ANNOUNCEMENT_VERSION = '3\.1\.13'/);
  assert.equal(JSON.parse(manifestSource).announcedApplicationVersion, "3.1.13");
  ...
  assert.equal("3.1.12" === "3.1.13", false, "le MSIX précédent doit être refusé");
  assert.equal("3.1.13" === "3.1.13", true, "seul le MSIX annoncé doit franchir le verrou");
  assert.equal("3.1.14" === "3.1.13", false, "un MSIX futur non annoncé doit être refusé");
...
test("le SSE adapte sa cadence, reprend par identifiant et désynchronise les reconnexions", async () => {
  ...
  assert.match(online, /XAR_SSE_FLUSH_PADDING_BYTES = 8192/);
  assert.match(stream, /ini_set\('output_buffering', '0'\)/);
  assert.match(stream, /ini_set\('implicit_flush', '1'\)/);
  assert.match(stream, /ob_implicit_flush\(true\)/);
  assert.match(stream, /str_repeat\(' ', XAR_SSE_FLUSH_PADDING_BYTES\)/);
  const writer = online.slice(online.indexOf("function writeOnlineEvent"), online.indexOf("function refreshOnlineConnectionRecord"));
  assert.match(writer, /str_repeat\(' ', XAR_SSE_FLUSH_PADDING_BYTES\)/);
...
test("l’ancien état global est en lecture seule et les commandes sont ciblées", async () => {
  ...
  assert.match(command, /\['ensure-player', 'admin\.character\.delete', 'token\.move', 'tokens\.transform', 'token\.clone', 'token\.conditions\.update', 'character\.conditions\.update', 'token\.resource\.adjust', 'action\.undo', 'token\.attack', 'token\.attack\.oppose', 'token\.attack\.resolve', 'ping'\]/);
...
test("les PV et le mana d’un token sont ajustés atomiquement sans élargir les données publiques", async () => {
  ...
  const adjustment = online.slice(online.indexOf("function applyOnlineTokenResourceAdjustment"), online.indexOf("function onlineAppendAppliedDamageAction"));
  assert.match(command, /\$isGm \? trim\(\(string\) \(\$arguments\['sceneId'\]/);
  assert.match(adjustment, /onlineTokenControllerIdFromRecords/);
  assert.match(adjustment, /\$characterIdFallback !== null/);
  assert.match(adjustment, /resource_target_changed/);
  assert.match(adjustment, /resource_character_missing/);
  assert.match(adjustment, /\!\$isGm && \(\$effectiveControllerId/);
  assert.match(adjustment, /\(\$token\['hidden'\] \?\? false\) === true/);
  assert.match(adjustment, /\$minimum = \$resource === 'hp' \? -1000000000 : 0[\s\S]*?\$current = max\(\$minimum, min\(\$maximum, \$previous \+ \$requestedDelta\)\)/);
  assert.match(adjustment, /\$resourceRepaired = \$storedResource != \$previous/);
  assert.match(adjustment, /\$resourceChanged = \$appliedDelta != 0 \|\| \$resourceRepaired/);
  assert.match(adjustment, /!\$resourceChanged && !\$allowNoopAtLimit/);
  assert.match(adjustment, /\$pulse = \$appliedDelta != 0 \? \[/);
  assert.match(adjustment, /if \(\$resourceChanged\) \{/);
  assert.match(adjustment, /applicationCharacterTokenDomainRecords\(\$connection, \$characterId\)/);
  assert.match(adjustment, /queueOnlineDomainUpsert\(\$pending, \$records, \$characterKey, \$character\)/);
  assert.match(adjustment, /'resourcePulse' => \$pulse/);
  assert.match(adjustment, /'characterId' => is_array\(\$character\) \? \$characterId : ''/);
  assert.match(command, /invalid_resource_request/);
  assert.match(command, /resource_receipt_forbidden/);
  assert.match(command, /XAR_RESOURCE_RECEIPT_TTL_MILLISECONDS/);
  assert.match(command, /count\(\$resourceReceipts\) >= XAR_RESOURCE_RECEIPT_MAXIMUM/);
  assert.match(command, /\$receiptActionIndex = findEntryIndex/);
  assert.match(command, /validOnlineRollFormula\(\$formula\)/);
  assert.match(command, /onlineRollFormula\(\$formula\)/);
  assert.match(command, /substr\(\(string\) \$rolled\['breakdown'\], 0, 500\)/);
  assert.match(command, /'operation' => \$operation/);
  assert.match(command, /\$command === 'action\.undo'/);
  assert.match(command, /action_already_undone/);
  assert.match(command, /applyOnlineTokenResourceAdjustment[\s\S]*?-\$originalDelta/);
  ...
  assert.match(domains, /XAR_RESOURCE_RECEIPT_MAXIMUM = 1024/);
  assert.match(domains, /resourceReceipts'\] \?\? \[\], XAR_RESOURCE_RECEIPT_MAXIMUM/);
});
...
test("le rapprochement automatique répare aussi un compte déjà présent en double", async () => {
  ...
  const repair = online.slice(online.indexOf("function repairOnlineRosterOwnershipsOnRead"), online.indexOf("function cleanPlayerCharacter"));
  assert.match(repair, /SELECT id, username, display_name FROM accounts WHERE revoked_at IS NULL ORDER BY id/);
...
test("les domaines bornent aussi les structures imbriquées et les registres secondaires", async () => {
  ...
  assert.match(domains, /\$folderId === null \|\| \$folderId === ''/);
...
test("les capacités migrées résolvent aussi les collisions de suffixes existants", async () => {
  const online = await read("api/v1/online.php");
  const abilities = online.slice(online.indexOf("function normalizeOnlineAbilities"), online.indexOf("function normalizeOnlineD100Difficulty"));
  assert.match(abilities, /\$baseId\s*=/);
  assert.match(abilities, /for \(\$suffix = \(int\) \$index \+ 1; isset\(\$seen\[\$id\]\); \$suffix \+= 1\)/);
  assert.match(abilities, /120 - strlen\(\$suffixText\)/);
  const linked = online.slice(online.indexOf("function normalizeOnlineLinkedTokens"), online.indexOf("function migrateOnlineCombatCharacterPayload"));
  assert.match(linked, /array_slice\(\$value, 0, 200\)/);
  assert.match(linked, /for \(\$suffix = \(int\) \$index \+ 1; isset\(\$seen\[\$id\]\); \$suffix \+= 1\)/);
  assert.match(linked, /120 - strlen\(\$suffixText\)/);
  assert.match(linked, /normalizePersistedImageReference/);
  assert.match(online, /\$current\[\$key\] = normalizeOnlineLinkedTokens\(\$patch\[\$key\]\)/);
});
...
test("plusieurs MJ sont sérialisés par transaction sans verrou de session global", async () => {
  ...
  assert.match(domains, /protectApplicationDomainAgainstStaleEntityWrite/);
  assert.match(patch, /prepareApplicationDomainUpsert\(\$key, \$change\['payload'\] \?\? null, \$current, true\)/);
