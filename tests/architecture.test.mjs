...
test("le backend 0.12.28 conserve la file Codex partagée et la migration révisionnée 1.15", async () => {
  const [index, domains, manifest] = await Promise.all([read("api/v1/index.php"), read("api/v1/domains.php"), read("manifest.json")]);
  assert.match(index, /XAR_BACKEND_VERSION = '0\.12\.28'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-2-5-14-release-20260831-1'/);
  ...
  assert.equal(JSON.parse(manifest).backendVersion, "0.12.28");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "2.5.14");
...
test("les calques PV sont publics en lecture seule, stables et gérés uniquement par le MJ", async () => {
  ...
  assert.match(online, /'size' => \(float\) \(\$token\['size'\] \?\? 45\)/);
});
...
test("seule la version MSIX annoncée peut utiliser l’API", async () => {
  ...
  assert.match(index, /XAR_RELEASE_ANNOUNCEMENT_VERSION = '2\.5\.14'/);
  assert.equal(JSON.parse(manifestSource).announcedApplicationVersion, "2.5.14");
  ...
  assert.equal("2.5.13" === "2.5.14", false, "le MSIX précédent doit être refusé");
  assert.equal("2.5.14" === "2.5.14", true, "seul le MSIX annoncé doit franchir le verrou");
  assert.equal("2.5.15" === "2.5.14", false, "un MSIX futur non annoncé doit être refusé");
