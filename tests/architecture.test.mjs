...
test("le backend 0.15.13 conserve la file Codex, porte le schéma de session 16 et le domaine chance", async () => {
  const [index, domains, manifest] = await Promise.all([read("api/v1/index.php"), read("api/v1/domains.php"), read("manifest.json")]);
  assert.match(index, /XAR_BACKEND_VERSION = '0\.15\.13'/);
  assert.match(index, /XAR_BACKEND_BUILD = 'client-3-2-13-media-targeting-stream-light-candidate-20260911-1'/);
  ...
  assert.equal(JSON.parse(manifest).backendVersion, "0.15.13");
  assert.equal(JSON.parse(manifest).announcedApplicationVersion, "3.2.13");
...
test("la santé reste publique mais seule la version courante peut se connecter", async () => {
  ...
  assert.equal(manifest.announcedApplicationVersion, "3.2.13");
  assert.deepEqual(manifest.allowedApplicationVersions, ["3.2.13"]);
  ...
  assert.match(readme, /3\.2\.12 \/ 3\.2\.13 \/ 3\.2\.14 = 426 \/ 401 \/ 426/);
