...
checkPolicy($policy['allowedVersions'] === ['3.2.13'], 'Seule la version courante est admise.');
checkPolicy($policy['minimumVersion'] === '3.2.13' && $policy['latestVersion'] === '3.2.13', 'La politique annonce la version à installer.');
...
foreach (['gm', 'player'] as $mode) {
    ...
    foreach (['', '3.1.13', '3.2.9', '3.2.10', '3.2.11', '3.2.12', '3.2.14', '3.3.0', '99.0.0', '3.2.13-beta', '3.2.13+local', '03.2.13', '3.2.12,3.2.13'] as $version) {
        $_SERVER['HTTP_X_XAR_CLIENT_VERSION'] = $version;
        try { requireSupportedClient($connection, []); throw new RuntimeException("Version admise à tort : $version"); }
            ...
            checkPolicy($response->body['allowedVersions'] === ['3.2.13']
                && $response->body['latestVersion'] === '3.2.13'
