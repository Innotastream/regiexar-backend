...
checkPolicy($policy['allowedVersions'] === ['3.2.2', '3.2.3', '3.2.4'], 'Seules les trois versions décidées sont admises.');
checkPolicy($policy['minimumVersion'] === '3.2.2' && $policy['latestVersion'] === '3.2.4', 'Le launcher ancien peut entrer et voit la nouvelle version.');
...
foreach (['gm', 'player'] as $mode) {
    ...
    foreach (['3.2.2', '3.2.3', '3.2.4'] as $version) {
    ...
    foreach (['', '3.1.13', '3.2.0', '3.2.1', '3.2.5', '3.3.0', '99.0.0', '3.2.2-beta', '3.2.3+local', '3.2.4+local', '03.2.4', '3.2.2,3.2.3,3.2.4'] as $version) {
        $_SERVER['HTTP_X_XAR_CLIENT_VERSION'] = $version;
        try { requireSupportedClient($connection, []); throw new RuntimeException("Version admise à tort : $version"); }
            ...
            checkPolicy($response->body['allowedVersions'] === ['3.2.2', '3.2.3', '3.2.4'] && $response->body['code'] === 'client_update_required', 'Le refus décrit la même politique.');
        }
    }
}
