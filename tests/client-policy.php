...
checkPolicy($policy['allowedVersions'] === ['3.3.1'], 'Seule la version courante est admise.');
checkPolicy($policy['minimumVersion'] === '3.3.1' && $policy['latestVersion'] === '3.3.1', 'La politique annonce la version à installer.');
...
foreach (['gm', 'player'] as $mode) {
    ...
    foreach (['', '3.1.13', '3.2.9', '3.2.10', '3.2.11', '3.2.12', '3.2.13', '3.2.14', '3.2.15', '3.2.16', '3.2.17', '3.2.18', '3.2.19', '3.2.20', '3.3.0', '3.3.2', '99.0.0', '3.3.1-beta', '3.3.1+local', '03.3.1', '3.3.0,3.3.1'] as $version) {
        $_SERVER['HTTP_X_XAR_CLIENT_VERSION'] = $version;
        try { requireSupportedClient($connection, []); throw new RuntimeException("Version admise à tort : $version"); }
            ...
            checkPolicy($response->body['allowedVersions'] === ['3.3.1']
                && $response->body['latestVersion'] === '3.3.1'
