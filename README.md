# Backend OVH — Régie du Seuil 0.15.5

Le candidat 0.15.5 autorise le MJ à choisir comme attaquant et comme cible deux pions visibles distincts du niveau actif, y compris créature contre créature ou pion joueur utilisé par le MJ. Les droits Joueur restent inchangés. Le journal MJ développe les calculs d’attaque, d’opposition et de dégâts jusqu’à l’armure absorbée ; Discord reçoit une synthèse sans valeur d’armure ni PV privés, avec les bonus, malus et modificateurs simples.

Les résultats remarquables ne concernent que les statistiques et la Chance : 1, 11, 22, 33 et 44 sont des réussites critiques, 55 une réussite spéciale, 66, 77, 88, 99 et 100 des échecs critiques. Ils brisent immédiatement une opposition, mais restent sans effet irréversible jusqu’à validation explicite du MJ. Cette évolution accompagne le client 3.2.5. Le build candidat est `client-3-2-5-combat-ruler-stream-candidate-20260909-1`. La politique de connexion reste une liste fermée : 3.2.2, 3.2.3, 3.2.4 et 3.2.5 sont autorisées, la version annoncée est 3.2.5 et toute autre version reste refusée.
...
Les copies PHP et leurs SHA historiques restent disponibles pour qualification dans le workflow du dépôt application. La production demeure en 0.15.4 tant qu’aucun déploiement 0.15.5 n’a été explicitement demandé ; préparer ou publier ce candidat ne l’aligne pas automatiquement.
...
La **0.15.4 / client annoncé 3.2.4** est la production contrôlée avant cette continuation, build `client-3-2-4-reversible-history-placement-candidate-20260908-1`. La 0.15.5 avance directement depuis ce main, conserve temporairement 3.2.2 à 3.2.4, ajoute **3.2.5** et l’annonce comme version la plus récente.
...
Ce candidat ajoute les sources de lumière par scène et niveau : 40 au maximum, portée de 1 à 40 cases (8 par défaut), relais déclenché uniquement par la vision stricte, hors murs et brouillard. Les chaînes sont déterministes ; les vues publiques et les attaques utilisent le même calcul et ne révèlent aucune source hors vue. Le MJ peut traverser les murs pendant un placement ; la destination doit rester libre pour toute l’empreinte du pion ou du groupe. Le trajet Joueur reste soumis aux collisions, avec contournement assisté dans les zones visibles. La taille par défaut passe à 40, sans modifier les tailles explicites. Les schémas session **16**, fiche **4**, domaines **1** et MySQL **18** restent inchangés.

PHP est indisponible dans l’environnement local de préparation ; les tests Node ne remplacent pas l’exécution PHP réelle du workflow. Après qualification du MSIX et déploiement OVH explicitement demandé, la future barrière publique attend **3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 / 3.2.5 / 3.2.6 = 426 / 401 / 401 / 401 / 401 / 426**. Tant que ce déploiement n’a pas lieu, la production et son contrôle 0.15.4 restent inchangés. Statut conservé : **livraison incomplète — alerte launcher non recettée**.
