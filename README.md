# Backend OVH — Régie du Seuil 0.14.7

La 0.14.7 annonce le MSIX `3.1.7` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Elle conserve le schéma de session 14 et la migration MySQL 016 introduits avec les types de dégâts, les armures physique et magique, la perception temporelle et les demandes d’attaque hors combat. `token.attack` contrôle côté serveur le propriétaire de l’attaquant, la visibilité de la cible, la formule, la statistique, les modificateurs et l’armure. En combat, les PV sont modifiés immédiatement et propagés à la fiche et aux tokens suiveurs. Hors combat, aucun PV ne change avant `token.attack.resolve` : seul un MJ peut appliquer la demande et l’approbation exige `confirmed=true`, issu de la case à cocher explicite. Les décisions sont un enum fermé, une cible à 0 PV est refusée, les files pleines échouent explicitement et les reçus idempotents actifs ne sont plus évincés après 256 attaques. Le webhook Discord des jets ne reçoit un résumé qu’après une perte effective de PV. L’application 3.1.7 rend visible toute l’épaisseur du premier corps de mur rencontré, y compris aux angles, aux extrémités arrondies et sur les ratios extrêmes, avant de commencer son masque noir juste derrière ; un second mur séparé et un obstacle hors portée ne sont pas révélés. Le masque de collision autoritaire, le recul après contact, les masques à 512 cellules, la projection filtrée, le relais SSE mutualisé et la fusion atomique des domaines restent inchangés.
