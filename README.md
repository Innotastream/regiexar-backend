# Backend OVH — Régie du Seuil 0.12.24

La 0.12.24 annonce le MSIX `2.5.10` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Une nouvelle connexion issue d'un ancien MSIX est donc refusée immédiatement. Elle conserve la fusion atomique des patchs partiels de `resources`, `stats` et `fatigue` afin qu’aucune modification de fiche ne puisse effacer les sous-champs voisins. Tout token visible expose sa fiche tactique en lecture seule, tandis que les notes non partagées, les notes secrètes MJ et tous les droits de jet, déplacement ou mutation restent protégés. Elle conserve les optimisations de simultanéité de la 0.12.16, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
