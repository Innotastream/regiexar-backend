# Backend OVH — Régie du Seuil 0.14.9

La 0.14.9 annonce le MSIX `3.1.9` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Elle conserve le schéma de session 15 et la migration MySQL 017. `token.attack` contrôle toujours côté serveur le propriétaire de l’attaquant, la visibilité de la cible, la formule, la statistique, les modificateurs et l’armure. `token.attack.oppose` recalcule le contrôleur courant de la cible, remet une demande réattribuée au bon joueur, limite son affichage à la scène visible, interdit qu’un attaquant défende sa propre nouvelle cible et refuse une cible déjà à 0 PV avant tout nouveau dé. Les reçus longs restent attachés aux attaques en attente, sont renouvelés après résolution et ne peuvent être rejoués que par leur intervenant initial. En combat, les PV sont modifiés dans la transaction ; hors combat, aucun PV ne change avant `token.attack.resolve` et son `confirmed=true` explicite. Deux réponses concurrentes ne peuvent pas appliquer deux fois une attaque. Le journal privé MJ conserve ATK, OPP et DMG brut présumé. Discord reçoit les résultats résolus avec de vraies lignes, mais jamais l’armure chiffrée, les dégâts finaux ou les PV. Les vues Joueur ne reçoivent ni le journal MJ ni l’armure des adversaires. Le masque de collision autoritaire, la vision avec corps de mur visible, le recul après contact, les masques à 512 cellules, le relais SSE mutualisé et la fusion atomique des domaines restent inchangés.
