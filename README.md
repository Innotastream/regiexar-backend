# Backend OVH — Régie du Seuil 0.14.2

La 0.14.2 annonce le MSIX `3.1.2` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Une nouvelle connexion issue d'un ancien MSIX est donc refusée immédiatement. Le client 3.1.2 conserve le schéma de session 13, 50 px comme taille normale de token, une grille de 50 px par défaut et les gabarits 30/50/70/100. Brume, murs et réglage de vision sont conservés séparément pour chacun des trois niveaux ; leur combinaison peut aussi être enregistrée dans quarante préréglages partagés strictement validés. Les menus fog et murs sont désormais exposés directement au-dessus de la map, les masques vides sont canonisés et les détails fins restent projetés lors d’une adaptation de ratio extrême. Les déplacements sont liés à la scène capturée au début du geste et contrôlés sur tout leur segment avec dilatation selon le gabarit du pion : un saut réseau ne traverse pas une cloison et une ouverture trop étroite reste fermée. Un joueur recouvert après coup reste immobilisé, tandis que le MJ peut extraire son token. La projection calcule pour chaque joueur l’union des champs de vision de ses tokens, bornée en cases et arrêtée par les murs ; les adversaires et signaux sous la brume, hors portée ou dans l’ombre sont retirés côté serveur. Les niveaux inactifs et le masque brut des murs ne sont jamais transmis. Les masques et préréglages mal formés ou surdimensionnés sont refusés avant écriture. Le backend conserve le relais SSE, la fusion atomique des patchs partiels, les optimisations de simultanéité, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
