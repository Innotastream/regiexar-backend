# Backend OVH — Régie du Seuil 0.13.0

La 0.13.0 annonce le MSIX `3.0.0` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Une nouvelle connexion issue d'un ancien MSIX est donc refusée immédiatement. Le client 3.0.0 utilise 50 px comme taille normale de token et les gabarits 30/50/70/100 ; la projection backend applique aussi 50 px lorsqu’une ancienne donnée ne renseigne aucune taille. Le brouillard de guerre est conservé sous forme de masque binaire borné pour chacun des trois niveaux. Seul le masque actif est transmis ; les adversaires et signaux couverts sont retirés côté serveur, tandis que le propriétaire conserve son propre pion visible. Les masques mal formés ou surdimensionnés sont refusés avant écriture. Le backend conserve le relais SSE, la fusion atomique des patchs partiels, les optimisations de simultanéité, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
