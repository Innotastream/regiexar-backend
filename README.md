# Backend OVH — Régie du Seuil 0.12.14

La 0.12.14 neutralise les faux conflits de documents dus au seul ordre des propriétés JSON et aux valeurs de compatibilité vides ajoutées par le client (`resourcePulse: null`, `folders: []`, `folderId: null`). Une requête sémantiquement identique est désormais reconnue comme sans effet avant le contrôle de révision : consulter la table ne peut plus transformer sa normalisation locale en édition concurrente. Elle conserve la compatibilité des anciens médias non classés et continue d’annoncer le candidat MSIX `2.5.2`, construit et techniquement contrôlé, comme unique version applicative autorisée par l’API ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Cette annonce ne prétend pas que le paquet est déjà installable depuis le Store.
