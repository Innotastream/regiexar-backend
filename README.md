...
## Livraison précédente conservée : 0.17.5

Client exact **3.3.10**, build `client-3-3-10-ruler-mj-sync-candidate-20260923-1`. Politique alignée sur la règle Joueur masquée après glissement et le signalement de l'envoi MJ. Les règles métier et diagnostics privés restent acquis. MySQL reste en version **21**, sans migration.
...
## Livraison précédente conservée : 0.17.1

Client exact **3.3.6**, build `client-3-3-6-live-map-sync-candidate-20260922-1`. Mise à jour de la politique de connexion pour les corrections des mouvements et menus. Le lancement des capacités accepte une fatigue supérieure au maximum et applique le malus de statistique strictement au-delà de la moitié du maximum. Les capacités rechargées par repos acceptent 1 à 100 utilisations réussies ; une réutilisation au cours du même tour reste permise si l’option est activée et le repos redevient obligatoire après ce tour quand le plafond est atteint. Repos court ou long libère le compteur par suppression ciblée du minuteur. Aucun changement de schéma ni de migration ; MySQL reste en version **21**. Le backend 0.17.0 et son historique sont conservés ci-dessous.

## Livraison précédente conservée : 0.17.0
...
Le backend 0.15.17 a été publié au commit `c9b3a25733a765145de9a5e02de47e5353bd00ef`, mais son workflow `34690687493` a échoué sur un contrat PHP. Il reste une pièce historique immuable et n’a jamais été déployé ; la 0.15.18 le remplace sans réécriture.

Les créations de fiches et minuteurs, les clones, les jets ordinaires ou tactiques, les ajustements de ressources et les conversations/générations du Studio disposent maintenant de reçus persistants. Une reprise avec la même référence et la même requête restitue le premier résultat sans nouvelle mutation, nouveau dé, nouvelle dépense, nouvelle activité ni second envoi Discord ; une référence réutilisée pour une requête différente répond `409`. L’absence de référence reste temporairement acceptée pour le client 3.2.14 encore en production jusqu’au basculement.

La reprise de propriété historique est fermée sur les identités explicitement déclarées. Aucun compte, personnage ou token n’est attribué par simple égalité de nom ; une ambiguïté ne modifie rien et reste diagnostiquable. Les présences sans fiche, les préférences et les entrées de roster valides ne sont plus supprimées par une lecture de santé ou d’état. Un minuteur ne peut plus être réutilisé avant son tour de disponibilité.
...
Qualification locale du candidat 0.15.20 : tests Node **48/48**. La syntaxe et les contrats PHP natifs restent des barrières obligatoires du workflow avant tout déploiement.
...
La santé et les informations de mise à jour Microsoft Store restent publiques pour tous les launchers, mais seule la version 3.2.20 peut ouvrir une session MJ/Joueur ou accéder aux données. Toute autre version reçoit `426 client_update_required` avec la version 3.2.20 à installer.
...
Après déploiement OVH, la barrière publique attend **3.2.18 / 3.2.19 / 3.2.20 / 3.2.21 / absente / mal formée = 426 / 426 / 401 / 426 / 426 / 426** tout en exigeant une santé publique accessible sans version cliente. Le 401 prouve seulement que la version courante franchit le verrou avant authentification. Le workflow GitHub avec PHP natif, l’installation Windows, l’écoute audio, Streamlabs et l’ultrawide réel restent des recettes distinctes.
...
La politique de connexion est définie exclusivement par `XAR_RELEASE_ALLOWED_CLIENT_VERSIONS` et la version annoncée `XAR_RELEASE_ANNOUNCEMENT_VERSION`, toutes deux reflétées dans le manifeste. Le bloc privé `client` ne peut pas désactiver cette protection. Cette livraison admet exactement `3.3.11`, avec `enforce=true`, `exactVersion=true`, `minimumVersion=3.3.11`, `latestVersion=3.3.11` et `allowedVersions=["3.3.11"]`. Les versions antérieures, futures et suffixées restent refusées pour la connexion.

Le refus de connexion ne bloque jamais la mise à jour : `/api/v1/health` reste public, annonce 3.3.11 et fournit l’identifiant Store. Un ancien launcher peut donc détecter la mise à jour et ouvrir Microsoft Store, mais il ne peut plus se connecter à la Régie avant installation du MSIX courant.

Avant toute remise du MSIX, qualifier l’artefact original, déployer le backend demandé puis vérifier la santé et la matrice `version effectivement en production avant déploiement / 3.3.11 / version future / absente / mal formée = 426 / 401 / 426 / 426 / 426`. Il reste interdit de remettre un MSIX plus récent que la santé publique. Chaque nouvelle version remplace explicitement cette valeur unique.
