...
Les copies PHP et leurs SHA sont préparées pour qualification dans le workflow du dépôt application ; la branche backend et OVH restent suspendus à la demande du propriétaire.
...
La **0.15.3 / client annoncé 3.2.3** est un candidat en cours de qualification. Build : `client-3-2-3-complete-lifecycle-audit-candidate-20260908-3`. Le propriétaire autorise exceptionnellement les connexions des seules versions **3.2.2 et 3.2.3**. La dernière base de production contrôlée est 0.15.2 / 3.2.2 ; la santé publique n’a pas été déclarée alignée sur le candidat.
...
PHP est indisponible dans l’environnement de préparation ; l’installation standard est bloquée par les restrictions système. Les tests Node ne remplacent pas l’exécution PHP réelle des copies candidates dans le workflow du dépôt application. Aucun push backend, déploiement OVH ou installateur remis n’est autorisé pendant cette pause. Une future activation demandée devra observer le contrôle public **3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 = 426 / 401 / 401 / 426**. Statut conservé : **livraison incomplète — alerte launcher non recettée**.
