# Backend OVH — Régie du Seuil 0.15.6

Le candidat 0.15.6 ajoute un environnement d’éclairage par niveau (`dark`, `normal`, `bright`) et une vision nocturne par fiche ou pion (`none`, `dim`, `full`). Sombre impose quatre cases, ou huit avec vision nocturne complète ; Normal conserve la portée propre à l’unité ; Éclairé impose seize cases. Le fondu reste visuel et ne modifie jamais l’autorité stricte des trajets. En Sombre et Normal, une lumière sans mur devant peut être perçue à trois fois la portée effective, puis relaie sa propre portée sans traverser murs ni brouillard.

Les lumières valent huit cases par défaut et conservent le relais existant. Le MJ choisit portée, couleur, icône (`torch`, `orb`, `wisp`) et portabilité. Un joueur ne peut prendre qu’une lumière visible, transportable, au contact d’un de ses pions valides ; le MJ peut aussi l’attribuer à une créature. Une seule lumière est portée par unité, les reçus sont idempotents, le changement de niveau suit le porteur et la mise hors combat ou la suppression pose la lumière à sa dernière position. La projection publique masque l’identité d’un porteur invisible.

Cette évolution accompagne le client 3.2.6. Le build candidat est `client-3-2-6-portable-lighting-launcher-candidate-20260909-1`. La politique de connexion reste une liste fermée : 3.2.3, 3.2.4, 3.2.5 et 3.2.6 sont autorisées, la version annoncée est 3.2.6 et toute autre version reste refusée. Elle retire 3.2.2 et conserve 3.2.5 dans les trois premières valeurs afin que son ancien parseur ne retranche plus sa propre version.
...
Les copies PHP et leurs SHA historiques restent disponibles pour qualification dans le workflow du dépôt application. La production contrôlée au départ de cette continuation est 0.15.5 ; préparer ou publier ce candidat ne l’aligne pas automatiquement sans déploiement OVH explicite et vérifié.
...
La **0.15.5 / client annoncé 3.2.5** est la production contrôlée avant cette continuation, build `client-3-2-5-combat-ruler-stream-candidate-20260909-1`. La 0.15.6 avance directement depuis son main exact, conserve temporairement 3.2.3 à 3.2.5, ajoute **3.2.6** et l’annonce comme version la plus récente.
...
Les sources de lumière restent bornées à 40 par niveau et à une portée de 1 à 40 cases. Les chaînes sont déterministes ; les vues publiques, les attaques et les déplacements utilisent les mêmes autorités. Le MJ peut traverser les murs pendant un placement ; la destination doit rester libre pour toute l’empreinte du pion ou du groupe. Le trajet Joueur reste soumis aux collisions, avec contournement assisté dans les zones strictement visibles. La taille par défaut reste 40. Les schémas session **16**, domaines **1** et MySQL **18** restent inchangés ; le schéma de fiche passe à **5** pour la vision nocturne.

PHP est indisponible dans l’environnement local de préparation ; les tests Node ne remplacent pas l’exécution PHP réelle du workflow. Après déploiement OVH explicitement demandé, la barrière publique attend **3.2.2 / 3.2.3 / 3.2.4 / 3.2.5 / 3.2.6 / 3.2.7 = 426 / 401 / 401 / 401 / 401 / 426**. Les 401 prouvent seulement le passage du verrou sans session. L’installation Windows, l’écoute audio, Streamlabs et l’ultrawide réel restent des recettes distinctes.
...
La politique de connexion est définie exclusivement par `XAR_RELEASE_ALLOWED_CLIENT_VERSIONS` et la version annoncée `XAR_RELEASE_ANNOUNCEMENT_VERSION`, toutes deux reflétées dans le manifeste. Le bloc privé `client` ne peut pas désactiver cette protection. Cette livraison admet exactement `3.2.3`, `3.2.4`, `3.2.5` et `3.2.6`, avec `enforce=true`, `exactVersion=false`, `minimumVersion=3.2.3`, `latestVersion=3.2.6` et `allowedVersions=["3.2.3","3.2.4","3.2.5","3.2.6"]`. Ce n’est pas une plage ouverte : les versions antérieures, futures et suffixées restent refusées.

Cette exception remplace pour cette livraison le verrou habituel sur une version unique. Les launchers 3.2.3 à 3.2.5 peuvent se reconnecter et proposent la mise à jour 3.2.6 ; leurs anciens défauts locaux restent présents jusqu’à leur remplacement. L’authentification, les droits et le renouvellement de génération des sessions demeurent obligatoires pour les quatre versions.

Avant toute remise du MSIX, qualifier l’artefact original, déployer le backend demandé puis vérifier la santé et la matrice `3.2.2 / 3.2.3 / 3.2.4 / 3.2.5 / 3.2.6 / 3.2.7 = 426 / 401 / 401 / 401 / 401 / 426`. Il reste interdit de remettre un MSIX plus récent que la santé publique. La prochaine évolution de la politique devra redéfinir explicitement sa liste.
