# Backend OVH — Régie du Seuil 0.14.4

La 0.14.4 annonce le MSIX `3.1.4` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Le client 3.1.4 conserve le schéma de session 13, 50 px comme taille normale de token, une grille de 50 px par défaut et les gabarits 30/50/70/100. Brume et murs restent propres aux trois niveaux et atteignent 512 cellules sur leur grand axe. Les déplacements restent liés à la scène capturée et contrôlés sur tout leur segment avec dilatation selon le gabarit. Le dernier point sûr est désormais sérialisé avec six décimales puis revérifié contre la hitbox : un joueur peut toujours reculer après un blocage sans franchir la cloison. Une tolérance inférieure à 0,02 cellule récupère seulement les contacts superficiels empoisonnés par l’arrondi de la 0.14.3 ; un pion réellement recouvert reste immobilisé et extractible par le MJ. La réponse fournit toujours la révision exacte du domaine token. La projection calcule ensuite une vision individuelle ou commune selon le choix du MJ ; les joueurs isolés ne partagent ni ne reçoivent les origines du groupe, et leur liste n’est jamais exposée. Les adversaires et signaux sous la brume, hors portée ou dans l’ombre sont retirés côté serveur. Les niveaux inactifs et le masque brut des murs ne sont jamais transmis. Les masques et préréglages mal formés ou surdimensionnés sont refusés avant écriture. Le backend conserve le relais SSE, la fusion atomique des patchs partiels, les optimisations de simultanéité, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
