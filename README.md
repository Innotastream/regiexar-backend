# Backend OVH — Régie du Seuil 0.14.3

La 0.14.3 annonce le MSIX `3.1.3` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Le client 3.1.3 conserve le schéma de session 13, 50 px comme taille normale de token, une grille de 50 px par défaut et les gabarits 30/50/70/100. Brume et murs restent propres aux trois niveaux, acceptent les anciens masques et peuvent désormais atteindre 512 cellules sur leur grand axe pour des traits fins plus réguliers. Les déplacements sont liés à la scène capturée au début du geste et contrôlés sur tout leur segment avec dilatation selon le gabarit du pion : un mouvement bloqué avance jusqu’au dernier point sûr au contact, sans traverser la cloison. Un joueur recouvert après coup reste immobilisé, tandis que le MJ peut extraire son token. La réponse d’un mouvement fournit aussi la révision exacte du domaine token. La projection calcule une vision individuelle ou commune selon le choix du MJ ; les joueurs isolés ne partagent ni ne reçoivent les origines du groupe, et leur liste n’est jamais exposée. Les adversaires et signaux sous la brume, hors portée ou dans l’ombre sont retirés côté serveur. Les niveaux inactifs et le masque brut des murs ne sont jamais transmis. Les masques et préréglages mal formés ou surdimensionnés sont refusés avant écriture. Le backend conserve le relais SSE, la fusion atomique des patchs partiels, les optimisations de simultanéité, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
