# Backend OVH — Régie du Seuil 0.12.28

La 0.12.28 annonce le MSIX `2.5.14` comme unique version applicative autorisée ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Une nouvelle connexion issue d'un ancien MSIX est donc refusée immédiatement. Le client 2.5.14 utilise 45 px comme taille normale de token et conserve les gabarits proportionnels 30/45/90/135 dans toutes les vues ; la projection backend applique aussi 45 px lorsqu’une ancienne donnée ne renseigne aucune taille. Il applique chaque révision SSE et matérialise déplacements, pings, variations de ressources, jets et transitions de combat même sur un second poste totalement inactif. Le flux SSE désactive explicitement les tampons PHP, remplit chaque événement jusqu’à 8 Kio pour franchir les relais FastCGI et borne sa lecture inactive à 750 ms afin d’éviter le retard groupé observé sur OVH. Le backend conserve la fusion atomique des patchs partiels de `resources`, `stats` et `fatigue` afin qu’aucune modification de fiche ne puisse effacer les sous-champs voisins. Tout token visible expose sa fiche tactique en lecture seule, tandis que les notes non partagées, les notes secrètes MJ et tous les droits de jet, déplacement ou mutation restent protégés. Il conserve les optimisations de simultanéité de la 0.12.16, le bail mobile du worker Studio, le signal de carte idempotent et la réparation contrôlée des anciens rattachements `player-<identifiant>`.
