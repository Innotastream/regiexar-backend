# Backend OVH — Régie du Seuil 0.15.4

Le candidat 0.15.4 rend l’effacement d’une génération strictement réversible côté conservation : `owner_hidden_at` la retire des vues du MJ, mais son message et son média continuent de se référencer mutuellement. Ni ce geste ni le nettoyage des médias orphelins ne programment alors le fichier pour suppression. L’action irréversible reste la suppression administrateur d’une discussion depuis `/studio`, qui efface son journal puis laisse la rétention contrôlée traiter les seuls fichiers réellement orphelins.

Cette évolution accompagne le client 3.2.4. Le build de livraison est `client-3-2-4-reversible-history-placement-candidate-20260908-1`. La politique de connexion reste une liste fermée : 3.2.2, 3.2.3 et 3.2.4 sont autorisées, la version annoncée est 3.2.4 et toute autre version reste refusée.
...
Les copies PHP et leurs SHA historiques restent disponibles pour qualification dans le workflow du dépôt application. La production 0.15.4 doit être alignée uniquement depuis ce `main` exact après contrôle du MSIX et du workflow backend.
...
La **0.15.3 / client annoncé 3.2.3** reste la base de production contrôlée avant basculement, build `client-3-2-3-complete-lifecycle-audit-candidate-20260908-3`. La livraison 0.15.4 conserve temporairement 3.2.2 et 3.2.3, ajoute **3.2.4** et l’annonce comme version la plus récente.
...
PHP est indisponible dans l’environnement local de préparation ; les tests Node ne remplacent pas l’exécution PHP réelle du workflow. Après qualification du MSIX et déploiement OVH, la barrière publique attend **3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 / 3.2.5 = 426 / 401 / 401 / 401 / 426**. Statut conservé : **livraison incomplète — alerte launcher non recettée**.
...
La politique de connexion est définie exclusivement par `XAR_RELEASE_ALLOWED_CLIENT_VERSIONS` et la version annoncée `XAR_RELEASE_ANNOUNCEMENT_VERSION`, toutes deux reflétées dans le manifeste. Le bloc privé `client` ne peut pas désactiver cette protection. Cette livraison admet exactement `3.2.2`, `3.2.3` et `3.2.4`, avec `enforce=true`, `exactVersion=false`, `minimumVersion=3.2.2`, `latestVersion=3.2.4` et `allowedVersions=["3.2.2","3.2.3","3.2.4"]`. Ce n’est pas une plage ouverte : les versions antérieures, futures et suffixées restent refusées.

Cette exception remplace pour cette livraison le verrou habituel sur une version unique. Les launchers 3.2.2 et 3.2.3 peuvent se reconnecter et proposent la mise à jour 3.2.4 ; leurs anciens défauts locaux restent présents jusqu’à leur remplacement. L’authentification, les droits et le renouvellement de génération des sessions demeurent obligatoires pour les trois versions.

Avant toute remise du MSIX, qualifier l’artefact original, déployer le backend demandé puis vérifier la santé et la matrice `3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 / 3.2.5 = 426 / 401 / 401 / 401 / 426`. Il reste interdit de remettre un MSIX plus récent que la santé publique. La prochaine évolution de la politique devra redéfinir explicitement sa liste.
