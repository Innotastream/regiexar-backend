# Backend OVH — Régie du Seuil 0.12.13

La 0.12.13 rétablit la compatibilité des documents existants lors de la création des premiers répertoires médias : les anciennes pistes non classées restent conservées et enregistrables, tandis qu’une piste classée doit toujours référencer un répertoire existant de son propre canal. Elle accepte aussi l’absence normale d’effet temporaire PV/mana (`resourcePulse: null`) afin qu’une modification de token ou un instantané de map ne soit plus rejeté. Elle continue d’annoncer le candidat MSIX `2.5.2`, construit et techniquement contrôlé, comme unique version applicative autorisée par l’API ; toute version antérieure ou supérieure non annoncée reçoit `426 client_update_required`. Cette annonce ne prétend pas que le paquet est déjà installable depuis le Store.
